    ...
    if (link) {
        if (window.location.pathname.split('/').length > 1)
            if (window.location.search.split('&').length > 1) {
                var paras = window.location.search.split('&');
                for (var i = 0; i < paras.length; i++) {
                    if (link.indexOf(paras[i].replace("?", "")) != -1 && link.indexOf(window.location.pathname.split('/')[1]+'?') != -1) {
                        ...
                        if ($('.nav-submenu.menu-content a.active').parents('.first').length > 0) {
                            $('#secondTitle').click();
                        } else
                            $('#firstTitle').click();
                        return false;
            ...
            else if (link.indexOf(window.location.search) != -1 && link.indexOf(window.location.pathname.split('/')[1] + '?') != -1) {
                $(this).parents().children('a').addClass('active');
                $(this).parents().parents().children('ul').css('display', 'block');
                $(this).addClass('active');
                $(this).parent().parent().parent().children('a').find('div').replaceWith('<div class="according-menu"><i class="fa fa-angle-down"></i></div>');
